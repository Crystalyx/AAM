...
public class MainMenuRegistry {
	...
	public static void createHelpFile(File dir) throws IOException
	{
		...
		pw.println("Your file must also contain a MenuType Integer representing the way your textureName gets rendered");
		...
		pw.println("2 Will render 1 small textureName given as a Texture String, repeating it all across the screen");
		pw.println("3 Will render N textures given in the Textures NBT Array with Texture String, leaving 1 textureName for the time given in the Textures NBT Array with the FadeTime integer");
		...
		pw.println("TextureMinX(Integer) and TextureMinY(Integer) is the start position on your textureName file(the file is considered to be 256x256, so making a bigger file will require you to perform additional calculations)");
		pw.println("TextureMaxX(Integer) and TextureMaxY(Integer) is the end position on your textureName file(the file is considered to be 256x256, so making a bigger file will require you to perform additional calculations)");
		pw.println("Alternatively you can use MinU, MinV, MaxU and MaxV doubles - these represent the texturePosition/textureSize. Only use if you know how that works. Allows for more custom HD textureName positioning");
		pw.println("XSize(Integer) and YSize(Integer) is the size of your textureName in the GUI.");
		...
		pw.println("Notice, that there are 2 images, due to minecraft.png actually having a split textureName");
		pw.println("");
		pw.println("{");
		pw.println("	menuName:\"I am a menu name!\",");
		pw.println("	menuDesc:\"I am a menu description!\",");
		...
		pw.println(" ButtonYOffset(Integer) is the amount of pixels the button textureName will be offset on the textureName file to get the button's disabled and mouseover textures");
				...
				if(detected.getName().endsWith(".dcmenu"))
				{
					...
					if(fis != null)
					{
						try
						{
							...
							NBTTagCompound tag = (NBTTagCompound) JsonToNBT.func_150315_a(st);
							if(validateNBT(tag))
							{
								tagsToMenu.add(tag);
