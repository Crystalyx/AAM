...
public class SoulAltarRenderer extends TileEntitySpecialRenderer
{
	...
	public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float f)
	{
		...
		if (tile instanceof TESoulAltar)
		{
			...
			{
				RenderUtils.renderItemStack_Full(t.getStackInSlot(0), 0, 0, 0, 0, 0, 0, tile.getWorldObj().getWorldTime() % 360, 0, 1, 1, 1, 0, -0.25F, 0, true);
			}
			...
			if (t.getStackInSlot(2) != null)
			{
				if (t.getStackInSlot(2).getItem() instanceof Artifact)
				{
					...
					RenderUtils.renderItemStack_Full(i, 0, 0, 0, 0, 0, 0, 90, 90, 1, 1, 1, 0.15F, -1F, 0, true);
				}
			}
			...
			for (int i = 0; i < ids.size(); i++)
			{
				...
				RenderUtils.renderItemStack_Full(t.getStackInSlot(ids.get(i)), 0, 0, 0, 0, 0, 0, 90, 90, 1, 1, 1, px + 0.125f, -1F, pz, true);
			}
		}
